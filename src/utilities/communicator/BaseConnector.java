...
public abstract class BaseConnector implements MessageListener {
	...
	/**JMS messages are sent and received using a Session. <br>
	 * We will create here a non-transactional session object. <br>
	 * If you want to use transactions you should set the first parameter to 'true'*/
	...
	/** listen to the default topic in the Class <br>
	 *  Destination represents here our topic 'ssh.CONTEXT' on the JMS server.<br>
	 *  You don't have to do anything special on the server to create it, it will be created automatically.<br>
	 *  MessageConsumer is used for receiving (consuming) messages<br>
	 *  Here we receive the message. <br>
	 *  By default this call is blocking, which means it will wait for a message to arrive on the queue/topic.<br>
	 *  */
	public void listen() {
        try {
	...
	/**Listen to the specific topic rather than default one declared in the Class<br>
	 * Destination represents here our topic 'ssh.CONTEXT' on the JMS server. <br>
	 * You don't have to do anything special on the server to create it, it will be created automatically. <br>
	 * MessageConsumer is used for receiving (consuming) messages <br>
	 * Here we receive the message.<br>
	 * By default this call is blocking, which means it will wait for a message to arrive on the queue/topic.<br>
	 * @param other_topic String
	 * */
	...
	/**Send to the default topic declared in the Class<br>
	 * MessageProducer is used for sending messages<br>
	 * (as opposed to MessageConsumer which is used for receiving them)*/
	...
	/**Send to the specific topic rather than default one declared in the Class <br>
	 * MessageProducer is used for sending messages <br>
	 * (as opposed to MessageConsumer which is used for receiving them)
	 * @param target_topic String*/
